...
class Oven:
    def __init__(self, input_size=51, optimizer=None, dropout=None, epoch=10, learning_rate=0.001, batch_size=100):
        ...
        self.lr_step = 0.05
        if not optimizer:
            self.optimizers_list = {"SGD": optim.SGD, "ADAM": optim.Adam, "RMSprop": optim.RMSprop,
    ...
    def train_and_vaildate(self, train_loader, test_loader):
        ...
        for epoch_i in range(self.epoch):
            ...
            self.pickle_save(path=f'results/{curr_accuracy}_epoch_{self.epoch_i}', filename=curr_accuracy,
                             info=f'accuracy: {curr_accuracy}')
    ...
    def test(self, test_loader, classes, file_name="y_test"):
        with open(file_name, "w") as output_file:
            for i, (data, labels) in enumerate(test_loader):
                ...
                output_file.write(
                    f"{os.path.basename(test_loader.dataset.spects[i][0])}, {classes[self.test_step(data)]}\n")
...
class Chef:
    ...
    def __init__(self, train_loader, test_loader, input_size=51, optimizer=None, dropout=None, epoch=10,
                 learning_rate=0.001, batch_size=100, label_size=2):
        """
        init the object
        :param input_size:
        :param optimizer: which oprimaizer to run
    ...
    def _recursiv_build_network_to_test(self, currnet, last_input_size):
        ...
        if self.depth_size + 1 <= len(currnet) / 3:
            ...
            if self.dev != "cpu":
                run_test.use_gpu()
            accuracy, lr, optimazier = run_test.best_Values_for_model_without_droupout(self.train_loader,
                                                                                       self.test_loader)
